...
class MaterialIssueRecord extends React.Component {
    ...
    downloadTable(event){
        ...
        if (_.isEmpty(selectedIds)) {
            ...
            this.setState({
                ...
                return fetch(`${config.apiUrl}/extract/downloadMir?projectId=${projectId}&screenId=${screenId}&unlocked=${unlocked}`, requestOptions)
    ...
    uploadTable(event){
        ...
        if(this.fileInput.current.files[0] && projectId && screenId && fileName) {
            ...
            return fetch(`${config.apiUrl}/extract/uploadMir`, requestOptions)
            .then(responce => responce.text().then(text => {
                const data = text && JSON.parse(text);
                if (!responce.ok) {
                    if (responce.status === 401) {
    ...
    render() {
        ...
        return (
            <Layout accesses={accesses} selection={selection} sidemenu={sidemenu} toggleCollapse={this.toggleCollapse} menuItem={menuItem}>
                {alert.message && !showSettings && !showCreatePt && !showModalUpload &&
                ...
                <div id="materialissuerecord" className={ (alert.message && !showSettings && !showCreatePt && !showModalUpload) ? "main-section-alert" : "main-section"}>
