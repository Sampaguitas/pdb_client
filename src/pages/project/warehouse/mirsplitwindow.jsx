...
class MirSplitwindow extends React.Component {
    ...
    uploadTable(event){
        ...
        if(this.fileInput.current.files[0] && projectId && screenId && fileName) {
            ...
            return fetch(`${config.apiUrl}/extract/uploadMirSplit`, requestOptions)
            .then(responce => responce.text().then(text => {
                const data = text && JSON.parse(text);
                if (!responce.ok) {
                    if (responce.status === 401) {
    ...
    render() {
        ...
        return (
            <Layout accesses={accesses} selection={selection} sidemenu={sidemenu} toggleCollapse={this.toggleCollapse} menuItem={menuItem}>
                {alert.message && !showSettings && !showSplitLine && !showModalUpload &&
                ...
                <div id="mirsplitwindow" className={ (alert.message && !showSettings && !showSplitLine && !showModalUpload) ? "main-section-alert" : "main-section"}>
