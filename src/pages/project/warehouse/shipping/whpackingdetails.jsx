...
class WhPackingDetails extends React.Component {
    ...
    downloadTable(event){
        ...
        if (_.isEmpty(selectedIds)) {
            ...
            this.setState({
                ...
                return fetch(`${config.apiUrl}/extract/downloadWhPackDetails?projectId=${projectId}&screenId=${screenId}&unlocked=${unlocked}`, requestOptions)
    ...
    uploadTable(event){
        ...
        if(this.fileInput.current.files[0] && projectId && screenId && fileName) {
            ...
            return fetch(`${config.apiUrl}/extract/uploadWhPackDetails`, requestOptions)
            .then(responce => responce.text().then(text => {
                const data = text && JSON.parse(text);
                if (!responce.ok) {
                    if (responce.status === 401) {
